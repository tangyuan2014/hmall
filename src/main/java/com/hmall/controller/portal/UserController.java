...
import com.hmall.service.impl.UserServiceImpl;
...
public class UserController {
    ...
    @RequestMapping(value = "getuserimfoamation.do", method = RequestMethod.POST)
    @ResponseBody
    public ServerResponse<User> getUserInfomation(HttpSession session) {
        User oldUser = (User) session.getAttribute(Const.CURRENT_USER);
        if (oldUser == null) {
            return ServerResponse.createByErrorMessageAndCode(ResponseCode.NEED_LOGIN.getCode(), "未登录，请强制登录");
        }
        ServerResponse<User> response = iUserService.getUserInformation(oldUser.getId());
        return response;
    }
